<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('compras.importar', $empresa) }}"
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            {{-- Selector de año/mes --}}
            <form method="GET" action="{{ route('compras.index', $empresa) }}"
            ...
            @else
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-700/50 text-left text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-right">Total</th><th class="px-3 py-2">Cuenta gasto</th><th class="px-3 py-2"></th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse ($documentos as $d)
                                <tr class="{{ $d->cuenta_gasto_id ? '' : 'bg-amber-50 dark:bg-amber-900/10' }}">
                                    <td class="px-3 py-2">{{ $d->nro }}</td>
                                    <td class="px-3 py-2">{{ $d->tipo_dte }}</td>
                                    <td class="px-3 py-2 font-mono">{{ $d->rut_proveedor }}</td>
                                    ...
                                    <td class="px-3 py-2">
                                        {{ $d->cuentaGasto?->nombre ?? '⚠ sin asignar' }}
                                    </td>
                                    <td class="px-3 py-2">
                                        {{ $d->estaCentralizado() ? '✔ centralizado' : '' }}
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="8" class="px-3 py-10 text-center text-gray-400">Sin documentos este mes.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                @if ($documentos->isNotEmpty() && $documentos->whereNull('comprobante_id')->isNotEmpty())
                    <form method="POST" action="{{ route('compras.centralizar', $empresa) }}">
                        @csrf
